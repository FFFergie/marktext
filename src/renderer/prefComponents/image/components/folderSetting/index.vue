<template>
  <section class="image-folder">
    ...
    <text-box :description="$t('pref.image.folder.globalImageFolder')" :input="imageFolderPath"
      :regexValidator="/^(?:$|([a-zA-Z]:)?[\/\\].*$)/" :defaultValue="folderPathPlaceholder"
      :onChange="value => modifyImageFolderPath(value)"></text-box>
    <div>
      <el-button size="mini" @click="modifyImageFolderPath(undefined)">{{ $t('common.open') }}...</el-button>
      <el-button size="mini" @click="openImageFolder">{{ $t('common.showInFolder') }}</el-button>
    </div>
    <compound>
      <template #head>
        <bool :description="$t('pref.image.folder.preferRelativeAssetsFolder')"
      ...
      <template #children>
        <text-box :description="$t('pref.image.folder.relativeImageFolderName')" :input="imageRelativeDirectoryName"
          :regexValidator="/^(?:$|(?![a-zA-Z]:)[^\/\\].*$)/"
          :defaultValue="relativeDirectoryNamePlaceholder"
          :onChange="value => onSelectChange('imageRelativeDirectoryName', value)"></text-box>
        <div class="footnote">
          {{ $t('pref.image.folder.filenamePlaceholder') }}
        </div>
      </template>
    </compound>
  </section>
