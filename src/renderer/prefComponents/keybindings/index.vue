  ...
  <div class="pref-keybindings">
    ...
    <section class="keybindings">
      <div class="text">
        ...
        {{ $t('pref.keybindings.moreText') }} <a class="link" @click="openKeybindingWiki">{{ $t('pref.keybindings.online') }}</a>.
      </div>
      <el-table
        :data="keybindingList"
        style="width: 100%"
      >
        ...
        <el-table-column prop="accelerator" :label="$t('pref.keybindings.keyCombination')" width="220">
        </el-table-column>
        <el-table-column fixed="right" :label="$t('common.options')" width="90">
          <template slot-scope="scope">
            ...
            <el-button @click="handleResetClick(scope.$index, scope.row)" type="text" size="small" :title="$t('common.reset')">
    ...
    <section class="footer">
      ...
      <el-button size="medium" @click="restoreDefaults">{{ $t('pref.keybindings.restoreDefaults') }}</el-button>
    </section>
...
export default {
  ...
  methods: {
      ...
      if (this.keybindingConfigurator && this.keybindingList.length > 0) {
        this.keybindingConfigurator.save()
          .then(success => {
            if (!success) {
              notice.notify({
                title: this.$t('pref.keybindings.failedToSave'),
                type: 'error',
                message: this.$t('common.unexpectedErrorWhileSaving')
              })
            }
          })
    ...
    handleDuplicateShortcut (id, accelerator) {
      notice.notify({
        title: this.$t('pref.keybindings.shortcutAlreadyInUse'),
        type: 'warning',
        message: this.$t('pref.keybindings.shortcutAlreadyInUseMessage', { accelerator })
      })
    },
