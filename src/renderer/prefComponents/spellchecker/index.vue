<template>
  <div class="pref-spellchecker">
    ...
    <compound>
      ...
      <template #children>
        <bool
          :description="$t('pref.spellchecker.hideMarks')"
        ...
        <bool
          v-show="isOsx"
          :description="$t('pref.spellchecker.autoDetectLanguage')"
          :bool="true"
          :disable="true"
        ></bool>
        <cur-select
    ...
    <div v-if="!isOsx && spellcheckerEnabled">
      <h6 class="title">{{ $t('pref.spellchecker.customDictionaryTitle') }}</h6>
      <div class="description">{{ $t('pref.spellchecker.customDictionaryDescription') }}</div>
      <el-table
        :data="wordsInCustomDictionary"
        :empty-text="$t('pref.spellchecker.noWords')"
        style="width: 100%"
      >
        <el-table-column prop="word" :label="$t('common.word')">
        </el-table-column>
    ...
    handleSpellcheckerLanguage (languageCode) {
      this.ensureDictLanguage(languageCode)
        ...
        .catch(error => {
          log.error(error)
          notice.notify({
            title: this.$t('pref.spellchecker.failedToSwitchLanguage'),
            type: 'error',
        ...
        ipcRenderer.invoke('mt::spellchecker-remove-word', selectedItem.word)
          .then(success => {
            if (success) {
              ...
              notice.notify({
                title: this.$t('pref.spellchecker.failedToRemoveCustomWord'),
                type: 'error',
